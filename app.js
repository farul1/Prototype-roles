const roles = {
  super_admin: {
    label: 'Super Admin', user: 'Syafarul Priwantoro', subtitle: 'Akses penuh seluruh modul SIPEDAM-TUS',
    menus: [
  ...
  organization: {
    label: 'Organisasi / Ormawa UKM', user: 'BEM TUS', subtitle: 'Pengajuan proposal kegiatan dan LPJ ORMAWA/UKM',
    menus: [
      { label: 'Dashboard', page: 'dashboard' },
      { label: 'Proses Pengajuan', page: 'process' },
...
const rows = [
  ...
  ['ORMAWA/UKM', 'UKM Robotika', 'Robotic Exhibition TUS', '08 Jul 2026', 'Ditolak', 'rejected']
];

const notifications = [
  ['Proposal baru masuk', 'BEM TUS mengirim proposal Festival Inovasi Mahasiswa.'],
...
function pageIntro(page) {
  ...
  return 'Halaman prototype mengikuti struktur role dan menu SIPEDAM-TUS.';
}
    ...
    <div class="detail-page-stack">
      ...
      <section class="tu-card detail-summary-card">
        ...
        <div class="detail-section">
          ...
          <div class="native-info-grid">
            ...
            <div><p>${isCompetition ? 'Penyelenggara' : 'Penanggung Jawab'}</p><strong>${isCompetition ? 'Indonesian Young Entrepreneur Forum' : 'Ketua BEM TUS'}</strong></div>
      ...
      <div class="detail-two-col">
        ...
        <section class="flow-panel flow-panel-rose">
          ...
          <div class="flow-panel-body">
            <div class="native-steps"><div><b>1</b><span>Upload proposal final</span></div><div><b>2</b><span>Cetak hardcopy</span></div><div><b>3</b><span>Serahkan ke SSC Lt.2 TUS</span></div></div>
          </div>
        </section>
      </div>
...
function settingsPage() {
  return `<div class="hero"><div><span class="eyebrow">Pengaturan Sistem</span><h1>Website Settings</h1><p>Atur logo, nama website, WhatsApp admin, notifikasi, force online, maintenance, dan link sinkronisasi rekap.</p></div></div><section class="panel" style="margin-top:18px"><div class="info-grid"><div class="info-box"><small>Nama Website</small><strong>SIPEDAM-TUS</strong></div><div class="info-box"><small>WhatsApp Admin</small><strong>+62 812 3456 7890</strong></div><div class="info-box"><small>Email Notification</small><strong>Aktif</strong></div><div class="info-box"><small>Force Online</small><strong>Nonaktif</strong></div></div><div style="height:16px"></div><button class="primary-btn" data-action="Simpan settings">Simpan Pengaturan</button></section>`;
}
...
function processPage() {
  const isCompetition = currentRole === 'mahasiswa_kompetisi' || currentRole === 'admin_kompetisi';
  const title = isCompetition ? 'Alur Pengajuan Kompetisi Mandiri' : currentRole === 'organization' ? 'Alur Pengajuan ORMAWA/UKM' : 'Alur Pengajuan Lengkap SIPEDAM-TUS';
...
function dataForContext(context) {
  if (context === 'lpj_ormawa') return [
    ...
    ['LPJ ORMAWA/UKM', 'UKM Musik', 'LPJ TUS Music Showcase', '12 Jul 2026', 'LPJ Revisi', 'revision'],
...
function taFlowPage() {
  ...
  return `<div class="hero"><div><span class="eyebrow">Buku TA</span><h1>Alur Prototype Utama SIPEDAM-TUS</h1><p>Halaman ini merangkum semua proses penting yang dibahas pada perancangan prototype TA.</p></div></div><section class="ta-flow-grid">${modules.map((m,i)=>`<article><b>${i+1}</b><strong>${m[0]}</strong><p>${m[1]}</p></article>`).join('')}</section>`;
}
